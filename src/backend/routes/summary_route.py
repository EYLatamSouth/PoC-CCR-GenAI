...
from src.backend.script.summary import summarization
from io import BytesIO
from zipfile import ZipFile

load_dotenv()

bp = Blueprint("summary", __name__, template_folder=folders.TEMPLATES,
               static_folder=folders.STATIC)

@bp.route('/upload_summary', methods=['GET', 'POST'])
def upload_summary():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            return redirect(url_for('summary.upload_summary'))
    
    return render_template('agents/summary.html')
...
def download_summaries():
    ...
    try:
        ...
        for txt_file_name in summary_files:
